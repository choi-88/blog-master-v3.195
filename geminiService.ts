...
export const generateBlogSystem = async (inputs: BlogInputs): Promise<BlogPost> => {
  ...
  const response = await fetch(URL, {
    ...
    body: JSON.stringify({
      contents: [{ 
        parts: [{ 
          text: `당신은 네이버 블로그 SEO 전문가입니다. "${inputs.productName}" 홍보글을 1,500자 이상의 장문으로 작성하세요. 제목은 "${inputs.mainKeyword}"로 시작하고 본문에 비교 표를 포함하세요. 반드시 아래의 JSON 형식으로만 응답하고, 마크다운 기호(예: \`\`\`json)를 포함하지 마세요.
          형식: {"title": "제목", "body": "1500자 본문", "imagePrompts": [{"nanoPrompt": "English keywords"}]}` 
        }] 
      }],
      generationConfig: { 
        // 💡 에러의 원인이었던 response_mime_type / responseMimeType 줄을 완전히 삭제했습니다.
  ...
  if (result.error) throw new Error(`구글 에러: ${result.error.message}`);

  let rawText = result.candidates?.[0]?.content?.parts?.[0]?.text || "";
  
  // 💡 AI가 혹시라도 ```json 같은 기호를 붙여서 줄 경우를 대비해 제거합니다.
